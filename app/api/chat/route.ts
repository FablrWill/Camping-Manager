...
function makeRunnableTools(): BetaRunnableTool<Record<string, unknown>>[] {
  return AGENT_TOOLS.map((tool) => ({
    ...
    run: async (input: Record<string, unknown>): Promise<string> => {
      return executeAgentTool(tool.name, input)
    },
  }))
}
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // BetaToolRunner with SDK-native max_iterations cap (CHAT-04)
          ...
          extractAndSaveMemory(fullAssistantText, userMessage).catch((err: unknown) =>
